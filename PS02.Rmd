...
Notice the sharp turns in the fitted line of the model where $df=20$. While the line fits these points better than the model with $df=5$, there is a risk of overfitting to our training set here. We are more interested in the model learning overall trends, instead of unique changes specific to this training set. We like the look of the model with $df=10$, shown below. 
...
The graph remains relatively smooth while also doing a good job of fitting the points in the training set. Our thinking is that this model learns the training set best without overfitting. 
